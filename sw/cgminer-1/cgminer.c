...
#if defined(HAVE_OPENCL) || defined(USE_USBUTILS) || defined(USE_SPONDOOLIES)
int nDevs;
#endif
#ifdef HAVE_OPENCL
...
#endif
#ifdef USE_SPONDOOLIES
char *opt_spondoolies_options = NULL;
#endif
#ifdef USE_USBUTILS
...
#endif
#ifdef USE_SPONDOOLIES
	OPT_WITH_ARG("--spondoolies-blala",
		     set_spondoolies_options, NULL, NULL,
		      "Set spondoolies something"),
#endif
#ifdef USE_AVALON
	     ...
	     char *hashshow, bool resubmit, char *worktime)
{
	...
	if (json_is_true(res) || (work->gbt && json_is_null(res))) {
		...
		printf("PROOF OF WORK RESULT: true (yay!!!)");
		if (!QUIET) {
			if (total_pools > 1)
		...
		printf("PROOF OF WORK RESULT: false (booooo)");
		if (!QUIET) {
			char where[20];
...
begin_bench:
	...
	for (i = 0; i < total_devices; ++i) {
		...
		for (j = 0; j < cgpu->threads; ++j, ++k) {
			...
			thr->id = k;
			thr->cgpu = cgpu;
			printf("---------> SETTING CGPU %p FOR THREAD %p\n", cgpu, thr);
			thr->device_thread = j;
	...
	while (42) {
		...
		this_t = time(NULL);
		if (this_t > last_t) {
			one_sec_spondoolies_watchdog((int)this_t - start_t);
			last_t = this_t;
		}
#endif
