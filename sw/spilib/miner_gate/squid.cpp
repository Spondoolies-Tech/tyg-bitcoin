...
void create_serial_pkt(uint32_t* d1, uint32_t* d2, 
						uint8_t reg_offset, uint8_t wr_rd, uint16_t dst_chip_addr,
						uint32_t reg_val, uint8_t general_bits) {	
	*d1  = reg_offset << 24;
	*d1 |= wr_rd << 23;
	*d1 |= dst_chip_addr << 7;
	*d1 |= reg_val >> 26;
...
void flush_spi_write() {
	if (hammer_serial_stack_size) {
		write_spi_mult(ADDR_SQUID_SERIAL_WRITE, hammer_serial_stack_size, hammer_serial_stack);
...
*/
	uint32_t d1;
	uint32_t d2;
	create_serial_pkt(&d1, &d2, offset, 0, addr, value, /*GENERAL_BITS_COMPLETION*/0);
	//printf("---> %x %x\n",d1,d2);
	push_hammer_serial_packet_to_hw(d1,d2);	
	if (enable_reg_debug) {
		printf("WRITE CMD %04x:%02x = %08x (%08x %08x)\n", addr,offset,value, d1, d2);
	}
}
...
int  wait_rx_queue_ready() {
	...
	//printf("Q STATUS:%x \n", q_status);
	while((++loops < 100) && ((q_status & BIT_STATUS_SERIAL_Q_RX_NOT_EMPTY) == 0)) {
        usleep(1);
...
uint32_t _read_reg_actual(uint32_t address, uint32_t offset) {
	...
	if (enable_reg_debug) {
		printf("READ RSP  %04x:%02x = %08x : (%08x %08x)\n", address,offset,value, values[0], values[1]);
	}
...
void squid_wait_hammer_reads() {
	...
	while (fpga_queue_status() == FPGA_QUEUE_WORKING) {
		...
		if (current_cmd_hw_queue_ptr == current_cmd_queue_ptr) {
			...
			/*
			uint32_t q_status = read_spi(ADDR_SQUID_STATUS);
			passert((q_status & BIT_STATUS_SERIAL_Q_RX_NOT_EMPTY) == 0, "ERROR: queue empty but qstatus busy");
			*/
			reset_hammer_queue();
			break;
		}
