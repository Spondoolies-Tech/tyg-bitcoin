...
RT_JOB* add_to_sw_rt_queue(const RT_JOB *work) {
	...
	assert(rt_queue[rt_queue_sw_write].work_state == WORK_STATE_FREE);
	...
	*work_in_queue = *work;
	...
	// to make sure...
	assert(work_in_queue->winner_nonce == 0);
	work_in_queue->work_id_in_hw = rt_queue_sw_write;
	rt_queue_sw_write = (rt_queue_sw_write+1)%0x100;
	rt_queue_size++;
    assert((rt_queue_sw_write - rt_queue_hw_done)%0x100 <= MAX_PACKETS_IN_RT_QUEUE);
    assert(rt_queue_size <= MAX_PACKETS_IN_RT_QUEUE);
...
int one_done_sw_rt_queue(RT_JOB *work) {
	if (rt_queue_size) {
		*work = rt_queue[rt_queue_hw_done];
		assert(rt_queue[rt_queue_hw_done].work_state == WORK_STATE_HAS_JOB);
